...
class PCDExporter:
    ...
    def add_color_using_cmap(self, value_per_point, cmap='jet'):
        cm = plt.get_cmap(cmap)
        normalizer = plt.Normalize(np.min(value_per_point), np.max(value_per_point))
        return self.add_color(cm(normalizer(value_per_point)))
